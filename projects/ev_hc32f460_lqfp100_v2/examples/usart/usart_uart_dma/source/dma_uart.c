...
static volatile uint16_t g_tx_w = 0; // 写指针
static volatile uint16_t g_tx_r = 0; // 读指针

static volatile uint16_t g_tx_dma_len = 0; // 当前 DMA 正在发送的长度
static volatile uint8_t g_tx_busy = 0;     // 1=DMA/USART 正在忙

static volatile uint8_t g_tx_dma_done = 0;
static volatile uint8_t g_tx_uart_done = 0;
...
static uint16_t ring_used(uint16_t r, uint16_t w)
{
    return (w >= r) ? (w - r) : (UART_TX_RING_SIZE - (r - w));
}

static uint16_t ring_free(uint16_t r, uint16_t w)
{
    // 留一个字节避免 r==w 歧义
    return (uint16_t)(UART_TX_RING_SIZE - 1u - ring_used(r, w));
}
...
static void Uart_TxKickLocked(void)
{
    // 已经在发就不动
    if (g_tx_busy)
        return;
    ...
    // 本次 DMA 只能发“从 r 到 buffer 末尾”的连续段
    uint16_t len = (w > r) ? (w - r) : (UART_TX_RING_SIZE - r);
    g_tx_dma_len = len;
    g_tx_busy = 1;
    ...
    DMA_SetSrcAddr(TX_DMA_UNIT, TX_DMA_CH, (uint32_t)&g_tx_ring[r]);
    ...
    volatile uint16_t c0 = DMA_GetTransCount(TX_DMA_UNIT, TX_DMA_CH);
    volatile uint32_t sr = USART_UNIT->SR; // 或你寄存器名
    (void)c0;
    (void)sr;
}

// 返回实际入队的字节数（ring 满了会少于 len）
uint16_t Uart_TxAsync(const uint8_t *data, uint16_t len)
{
    if (len == 0)
        return 0;
    ...
    __disable_irq(); // 临界区很短，只做拷贝/指针更新/可能kick一次

    uint16_t r = g_tx_r;
    uint16_t w = g_tx_w;
    uint16_t free = ring_free(r, w);
    if (free == 0)
    {
        __enable_irq();
        return 0;
    ...
    // 分两段拷贝：w->end, 0->...
    uint16_t tail = (uint16_t)(UART_TX_RING_SIZE - w);
    uint16_t n1 = (len < tail) ? len : tail;
    for (uint16_t i = 0; i < n1; i++)
        g_tx_ring[w + i] = data[i];

    uint16_t n2 = (uint16_t)(len - n1);
    for (uint16_t i = 0; i < n2; i++)
        g_tx_ring[i] = data[n1 + i];

    w = (uint16_t)((w + len) % UART_TX_RING_SIZE);
    g_tx_w = w;
    pushed = len;
...
{
    // DMA 搬完，但线还没一定发完
    // 先开 TCI 中断，等“最后 stop bit 发完”再推进队列
...
static void USART_TxComplete_IrqCallback(void)
{
    // 1) 推进读指针：消费掉刚才 DMA 发送的那段
    g_tx_r = (uint16_t)((g_tx_r + g_tx_dma_len) % UART_TX_RING_SIZE);
    ...
    // 3) 关掉 TCI 中断（避免空转）
    USART_FuncCmd(USART_UNIT, USART_INT_TX_CPLT, DISABLE);

    // 4) 如果队列还有数据，继续 kick 下一段；否则可以关 TX 省电
    if (g_tx_r != g_tx_w)
    {
        __disable_irq();
        Uart_TxKickLocked();
    ...
    {
        USART_FuncCmd(USART_UNIT, USART_TX, DISABLE); // 可选：不关也行
    }
}
...
void uart_dma_init(void)
{
    ...
    stcUartInit.u32CKOutput = USART_CK_OUTPUT_DISABLE; // USART_CK_OUTPUT_ENABLE;
